...
def weighted_mean(x_norm: np.ndarray, quality_df: pd.DataFrame) -> np.ndarray:
    frames = quality_df["avg_processed_frames"].to_numpy(dtype=np.float32)
    quality = quality_df["quality_score"].to_numpy(dtype=np.float32)
    valid_ratio = quality_df["valid_video_ratio"].to_numpy(dtype=np.float32)

    if np.max(frames) > 0:
        frames = frames / np.max(frames)

    weights = (0.6 * quality) + (0.3 * valid_ratio) + (0.1 * frames)
...
def largest_cluster_center(x_norm: np.ndarray, n_clusters: int = 3) -> np.ndarray:
    ...
    dominant_idx = np.where(labels == dominant_label)[0]
    return x_norm[dominant_idx].mean(axis=0)


def main() -> None:
    features_df, quality_df = load_person_profiles(OUTPUT_DIR)
    if features_df.empty:
        print("[WARN] No person_profile.json found in output folder.")
        return

    feature_cols = [col for col in features_df.columns if col != "person"]
    ...
    similarity_table = pd.DataFrame({"person": features_df["person"]})

    for method_name, pref_vec in methods.items():
        sims = compute_similarity(x_norm, pref_vec)
        similarity_table[method_name] = sims

        top_idx = np.argsort(sims)[::-1][:10]
        top_people = [
            {"person": features_df.iloc[i]["person"], "similarity": float(sims[i])}
            for i in top_idx
        ]
    ...
    report_path = OUTPUT_DIR / "preference_strategies_report.json"
    sim_csv_path = OUTPUT_DIR / "preference_strategy_similarity_scores.csv"

    with report_path.open("w", encoding="utf-8") as file:
        json.dump(report, file, ensure_ascii=False, indent=2)

    similarity_table.to_csv(sim_csv_path, index=False, encoding="utf-8")

    print(f"[DONE] Saved report: {report_path}")
    print(f"[DONE] Saved similarity table: {sim_csv_path}")
